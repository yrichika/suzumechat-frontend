export const en = new Map<string, string>([
  ['create-button', 'create chat channel'],
  ['input-label-channelName', 'Channel Name'],
  ['subtitle', 'Totally Anonymous and Private Web Chat!'],
  [
    'description',
    'No user account. No personal information. No stored messages. No history. In-browser message encryption!',
  ],
  ['feature-h', 'Features'],
  [
    'feature-item-1',
    'Without creating an account, you can start a chat room with anyone you want!',
  ],
  [
    'feature-item-2',
    'No personal info is necessary to create a chat channel(chat room).',
  ],
  [
    'feature-item-3',
    'No message is stored on the server. Messages are displayed only in your browser.',
  ],
  [
    'feature-item-4',
    "Messages are encrypted in your browser. (If you delete your chat secret key, we even can't recover your message!)",
  ],
  [
    'feature-item-5',
    'Messages will vaporize approximately every 20 seconds. And Only latest 10 messages will be displayed.',
  ],
  [
    'feature-item-6',
    "All data on the server is also encrypted or hashed. (All data except tables' primary keys and timestamps. But primary keys and timestamps have nothing to do with identifying who used or with messages you sent).",
  ],
  [
    'feature-item-7',
    "When you end a chat channel, it's impossible to recover the channel or messages.",
  ],
  ['feature-item-8', 'It is open source, and the code is public.'],
  [
    'feature-item-9',
    'If you find any problem, please let me know on Twitter or GitHub.',
  ],
  ['terms-of-service-title', 'Terms of Service'],
  // The section below is generated by a script not included in this project yet. Please do not edit the code below.
  [
    'chapter-0',
    "Sorry, I'm still working on it! If you would like to read the terms of service, please use any translation tool, and translate the Japanese version to any other language you can read.",
  ],
  ['chapter-h-1', ''],
  ['tos-1-1', ''],
  ['tos-1-2', ''],
  ['tos-1-3', ''],
  ['chapter-h-2', ''],
  ['tos-2-1', ''],
  ['tos-2-2', ''],
  ['tos-2-3', ''],
  ['tos-2-4', ''],
  ['tos-2-5', ''],
  ['tos-2-6', ''],
  ['tos-2-7', ''],
  ['tos-2-8', ''],
  ['chapter-h-3', ''],
  ['tos-3-1', ''],
  ['tos-3-2', ''],
  ['tos-3-3', ''],
  ['tos-3-4', ''],
  ['tos-3-5', ''],
  ['tos-3-6', ''],
  ['tos-3-7', ''],
  ['tos-3-8', ''],
  ['tos-3-9', ''],
  ['chapter-h-4', ''],
  ['tos-4-1', ''],
  ['tos-4-2', ''],
  ['chapter-h-5', ''],
  ['tos-5-1', ''],
  ['tos-5-2', ''],
  ['tos-5-3', ''],
  ['tos-5-4', ''],
  ['tos-5-5', ''],
  ['tos-5-6', ''],
  ['tos-5-7', ''],
  ['tos-5-8', ''],
  ['tos-5-9', ''],
  ['tos-5-10', ''],
  ['tos-5-11', ''],
  ['tos-5-12', ''],
  ['tos-5-13', ''],
  ['tos-5-14', ''],
  ['tos-5-15', ''],
  ['tos-5-16', ''],
  ['tos-5-17', ''],
  ['tos-5-18', ''],
  ['tos-5-19', ''],
  ['tos-5-20', ''],
  ['tos-5-21', ''],
  ['tos-5-22', ''],
  ['tos-5-23', ''],
  ['tos-5-24', ''],
  ['tos-5-25', ''],
  ['tos-5-26', ''],
  ['tos-5-27', ''],
  ['tos-5-28', ''],
  ['tos-5-29', ''],
  ['tos-5-30', ''],
  ['chapter-h-6', ''],
  ['tos-6-1', ''],
  ['tos-6-2', ''],
  ['tos-6-3', ''],
  ['tos-6-4', ''],
  ['tos-6-5', ''],
  ['tos-6-6', ''],
  ['chapter-h-7', ''],
  ['tos-7-1', ''],
  ['tos-7-2', ''],
  ['tos-7-3', ''],
  ['chapter-h-8', ''],
  ['tos-8-1', ''],
  ['tos-8-2', ''],
  ['tos-8-3', ''],
  ['tos-8-4', ''],
  ['tos-8-5', ''],
  ['tos-8-6', ''],
  ['chapter-h-9', ''],
  ['tos-9-1', ''],
  ['tos-9-2', ''],
  ['chapter-h-10', ''],
  ['tos-10-1', ''],
  ['tos-10-2', ''],
  ['tos-10-3', ''],
  ['tos-10-4', ''],
  ['chapter-h-11', ''],
  ['tos-11-1', ''],
  ['chapter-h-12', ''],
  ['tos-12-1', ''],
  ['tos-12-2', ''],
  ['chapter-h-13', ''],
  ['tos-13-1', ''],
  ['chapter-h-14', ''],
  ['tos-14-1', ''],
  ['chapter-h-15', ''],
  ['tos-15-1', ''],
  ['tos-15-2', ''],
  ['chapter-h-16', ''],
  ['tos-16-1', ''],
  ['chapter-h-17', ''],
  ['tos-17-1', ''],
  ['tos-17-2', ''],
  ['tos-created-at', ''],
])
